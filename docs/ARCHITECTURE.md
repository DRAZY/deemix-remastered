...
- **Localization (2.6.2):** every user-facing string goes through vue-i18n, `t()` in components and `i18n.global.t()` in stores and composables, with English as the fallback locale. Two invariants, both learned from #148: functional copy (toasts, labels, help text, status words) is never typed into a component, and a new key ships in all 21 locale files in the same commit. The exceptions are the app's own branded labels ("Transfer Rack", "Signal Deck", "AGGREGATE RATE", "Pull the signal") and its instrument-panel status readouts (LINK ESTABLISHED, LINK DOWN, Q:LINKED, Q:OFFLINE, LIVE, STORED, RECV, QUEUE, FAULT, HOLD, NO SIGNAL), which stay English by design. Controls and field labels styled the same way are NOT exempt: the rack's Folder, Clear and Abort buttons and the title bar's Region and Quality labels were translated in 2.6.3 (#148). `scripts/i18n-check.ts` enforces parity (every locale carries every English key) and flags hard-coded English in templates, toasts and attributes; CI runs it.
...
**Shared album context.** Folder structure (incl. `CD1`/`CD2` subfolders), folder/file naming, and album-level tags all derive from an album *context* object built by `buildAlbumContext` in `electron/services/albumContext.ts` — the **single source of truth**, used by every download path so they produce identical results. The same file holds `pinReleaseFields` (2.6.3): when a track inside an album download is replaced by an alternate version, release-level fields (album id and title, cover, release dates, barcode, label, copyright, genre) are re-applied from the requested track and from the context's `coverMd5`, while track-level fields (id, title, version, ISRC, gain, duration) stay the alternate's. Without it the alternate's cover and dates leaked into the album, which put three different covers into one Mötley Crüe album. It returns a copy and pins nothing when there is no album context (single track, playlist):
...
**Lyrics come from two Deezer endpoints.** `deezerAuth.getLyrics` asks the legacy gateway (`song.getLyrics`) first, which returns `LYRICS_TEXT` plus `LYRICS_SYNC_JSON`. Since 2026 that call has been returning text only for part of the newer catalog, so when the synced block is missing the service asks Deezer's GraphQL lyrics API (`pipe.deezer.com`, authenticated with a short-lived JWT minted from the session's arl and cached per arl) and maps `synchronizedLines` into the gateway shape. Every consumer (the `.lrc` writer, the SYLT frame, the FLAC `SYNCEDLYRICS` comment) reads that one shape, so the fallback is invisible past `getLyrics`. Guest sessions get no fallback because the GraphQL API needs a signed-in arl (#158).
