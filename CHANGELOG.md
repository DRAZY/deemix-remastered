...
- An album no longer comes down with mixed cover art when some of its tracks are alternate versions. The alternate's cover and release date used to leak in from whatever release it lived on, so Mötley Crüe's *Shout At The Devil (40th Anniversary)* arrived with three different covers, one of them from The Dirt Soundtrack, and a folder `cover.jpg` from the wrong release. In an album download, cover, folder cover, release date, album, barcode and label now stay those of the album you asked for. The track title and ISRC still describe the recording you actually received, and the Alternate version badge still says so. Single tracks and playlists are unchanged. Albums already on disk keep their mixed art until you download them again with overwrite on.
